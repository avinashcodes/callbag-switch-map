...
test('it creates a new source for every value emitted by input source', t=>{
	...
	const combineValues = (value1, value2) => {
		t.deepEqual([value1, value2], [3, 9], 'combineResults called with input source result and applied source result');
	...
	const outputSource = switchMap(x => fromPromise(doSomething(x)), combineValues)(inputSource);
	outputSource(0, (type,d) => {
		if(type === 1) {
			t.equal(d, "3 9", "Got only latest result");
		}
	});
...
test('it should stop emitting when sink unsubscribes', t=>{
	...
	const outputSource = switchMap(x => fromPromise(doSomething(x)))(inputSource);
	let talkback;
	outputSource(0, (type,d) => {
